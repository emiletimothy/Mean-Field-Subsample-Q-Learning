...
n = 3 # number of local agents
samples = 10 # number of samples
gamma = 0.9 # discount factor
T = 50 # number of learning steps
...
Q_function = Q_function(n=k, state_spaces=state_spaces, action_spaces=action_spaces, samples=samples, transition_system=transition_system, reward_system=reward_system, gamma=gamma)
Q_function.learn(steps=50)
...
for t in range(H):
    ...
    for i in range(n):
        ...
        state_idx = Q_function.get_state_idx(joint_state_for_i)
        # find the action that maximizes Q-value for this state
        best_action_idx = np.argmax(Q_function.Q[state_idx, :])
        best_joint_action = Q_function.joint_actions[best_action_idx]
    ...
    state_idx = Q_function.get_state_idx(joint_state_for_global)
    best_action_idx = np.argmax(Q_function.Q[state_idx, :])
    global_action = Q_function.joint_actions[best_action_idx][0]
...
# this is appended to the cumulative_rewards
